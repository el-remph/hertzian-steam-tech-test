...
class Test_Review_Stream(unittest.TestCase):
	async def ugly_setup(self):
		_, self.reviews = await foo.Review_Stream(1382330, self.n_max, foo.Review_Stream.Date_Type.UPDATED, ('2023-11-18', '2024-02-12')).nextbatch()
...
class Test_File_Output(unittest.TestCase):
	...
	def test_output(self):
		...
		try:
			...
			nmax = 50
			maxfiles = 3
			date_range = ('2023-11-18', '2024-02-12')
			min_date, max_date = [date.fromisoformat(d) for d in date_range]
			splitter = foo.Split_Reviews(steamid, date_range=date_range, per_file=nmax, max_files=maxfiles)
			splitter.loop()
			file_i = splitter.file_i
			self.assertTrue(file_i <= maxfiles, '{:d} > {:d}'.format(file_i, maxfiles))
			del splitter
			...
			for i in range(maxfiles):
				...
				for r in reviews:
					...
					self.assertFalse(newdate > curdate, 'dates out of order: {}, {}'.format(newdate, curdate))
					self.assertTrue(min_date <= newdate <= max_date, 'date out of bounds: {}'.format(newdate))
					if newdate < curdate:
						curdate = newdate
